...
function SelectPool()
	local index = GetRandomIntInRange(1,  #Config.Gold)

	for k,v in pairs(Config.Zones) do
		if v.Pos.x == Config.Gold[index].x and v.Pos.y == Config.Gold[index].y and v.Pos.z == Config.Gold[index].z then
			return k
		end
	end
end
...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('esx_goldminer:GiveItem')
		StartNPCJob()
		Done = true
	end
		...
		if NPCTargetPool ~= nil then
			...
			if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < 3 then
				...
				if IsControlJustReleased(1, Keys["E"]) and PlayerData.job ~= nil then
					TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_CONST_DRILL", -1, true)
...
AddEventHandler('esx_goldminer:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_goldminer:hasExitedMarker', function(zone)

	if zone == 'Sale' then
		TriggerServerEvent('esx_goldminer:stopSale')
	end
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_goldminer:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_goldminer:hasExitedMarker', LastZone)
			end
		end
	end
end)
				...
				if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
					...
					if PlayerData.job.name == Config.nameJob then
						...
						if CurrentAction == 'Sale' then
							TriggerServerEvent('esx_goldminer:startSale')
						end
